...
serve(async (req) => {
  ...
  try {
    ...
    const { type, imageBase64, textReport, roadNames, nodeNames } = await req.json();
...
2. For each mentioned road, what is the condition? (congested/clear/accident/construction)
3. Severity score for each road (1-100, where 1 is free-flowing and 100 is completely blocked)
4. Brief summary
        ...
        tools: [
          {
            ...
            function: {
              ...
              parameters: type === "traffic_report"
                ? {
                    type: "object",
                    properties: {
                      roads: {
